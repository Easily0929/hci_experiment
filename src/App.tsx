...
import { loadConfig, configToVoiceModelConfig } from './config';
import { buildTencentASRUrl } from './tencentASR';
...
const useEdgeSpeechRecognition = (
  ...
  // 使用正确的 TC3-HMAC-SHA256 签名算法生成腾讯云ASR URL
  const generateTencentCloudASRUrl = useCallback((
    secretId: string,
    secretKey: string,
    appId: string,
    params: {
    ...
    try {
      console.log('🔐 正在生成腾讯云ASR URL（TC3-HMAC-SHA256）...');
      
      const wsUrl = buildTencentASRUrl({
        appId,
        secretId,
        secretKey,
        engineModelType: params.engine_model_type || '16k_zh',
        voiceFormat: params.voice_format || '1',
        needvad: params.needvad || '1',
        filterDirty: params.filter_dirty || '0',
        filterModal: params.filter_modal || '0',
        filterPunc: params.filter_punc || '0',
        convertNumMode: params.convert_num_mode || '1',
      });

      console.log('✅ 腾讯云ASR URL 生成成功', {
        urlLength: wsUrl.length,
        urlPreview: wsUrl.substring(0, 100) + '...',
        hasAlgorithm: wsUrl.includes('TC3-HMAC-SHA256'),
        hasCredential: wsUrl.includes('credential='),
        hasSignature: wsUrl.includes('signature='),
      });

      return wsUrl;
    } catch (err: any) {
      console.error('❌ 生成腾讯云ASR URL失败:', err);
      setError(`生成连接URL失败: ${err.message}\n\n请检查：\n1. SecretId 和 SecretKey 是否正确\n2. AppId 是否正确\n3. crypto-js 库是否已安装`);
      return null;
    }
  }, []);
  ...
  const startTencentCloudASR = useCallback(async (isReconnect = false) => {
    ...
    try {
      ...
      // 使用正确的 TC3-HMAC-SHA256 签名算法生成 WebSocket URL
      let wsUrl: string | null = null;
      
      try {
        const secretKey = voiceModel.recognitionSecretKey;
        ...
        // 从 recognitionUrl 中提取 AppId
        const urlObj = new URL(voiceModel.recognitionUrl);
        ...
        console.log('🔍 签名参数检查:', {
          appId,
          secretId: voiceModel.recognitionKey ? voiceModel.recognitionKey.substring(0, 10) + '...' : '未设置',
          secretKeyLength: secretKey ? secretKey.length : 0,
          secretKeyPrefix: secretKey ? secretKey.substring(0, 5) + '...' : '未设置',
        });
        
        // 验证SecretId和SecretKey格式
        if (!voiceModel.recognitionKey || voiceModel.recognitionKey.length < 10) {
          throw new Error('SecretId 格式不正确，长度应至少为10个字符');
        }
        if (!secretKey || secretKey.length < 10) {
          throw new Error('SecretKey 格式不正确，长度应至少为10个字符。请检查 recognitionSecretKey 配置。');
        }
        
        // 使用 TC3-HMAC-SHA256 算法生成完整的 WebSocket URL
        wsUrl = generateTencentCloudASRUrl(
        ...
        if (!wsUrl) {
          throw new Error('生成 WebSocket URL 失败');
        }
        
        console.log('✅ WebSocket URL 已生成（使用 TC3-HMAC-SHA256）');
      } catch (err: any) {
        console.error('构建 WebSocket URL 失败:', err);
        setError(`构建连接 URL 失败: ${err.message}\n\n请检查：\n1. 识别服务 URL 格式是否正确（应包含 AppId）\n2. SecretId 和 SecretKey 是否正确配置\n3. crypto-js 库是否已安装（运行 npm install）`);
        setIsListening(false);
        return;
      }
      
      if (!wsUrl) {
        setError('WebSocket URL 生成失败');
        setIsListening(false);
        return;
      }
      
      console.log('🔌 连接腾讯云 WebSocket...', wsUrl.substring(0, 100) + '...');
      console.log('🔍 WebSocket URL 信息:', {
        urlLength: wsUrl.length,
        hasAlgorithm: wsUrl.includes('TC3-HMAC-SHA256'),
        hasCredential: wsUrl.includes('credential='),
        hasSignature: wsUrl.includes('signature='),
      ...
      ws.onmessage = (event) => {
        try {
          ...
          // 处理签名错误
          if (data.code !== 0 && (data.code === 4001 || data.code === 4002)) {
            const errorMsg = data.message || '未知错误';
            const currentSecretKey = voiceModel?.recognitionSecretKey || '';
            console.error('❌ 腾讯云签名错误:', {
              code: data.code,
              message: errorMsg,
              fullData: data,
            });
            
            setError(`❌ 签名错误 (${data.code}): ${errorMsg}\n\n可能的原因：\n1. SecretId 和 SecretKey 不匹配\n2. SecretKey 配置错误（请检查 recognitionSecretKey 字段）\n3. 签名算法实现问题\n\n请检查：\n- recognitionKey (SecretId): ${voiceModel?.recognitionKey ? voiceModel.recognitionKey.substring(0, 10) + '...' : '未设置'}\n- recognitionSecretKey (SecretKey): ${currentSecretKey ? '已设置（长度: ' + currentSecretKey.length + '）' : '未设置'}\n\n建议：\n1. 在腾讯云控制台重新生成 SecretId 和 SecretKey\n2. 确保 SecretId 和 SecretKey 是一对匹配的密钥\n3. 检查 recognitionSecretKey 字段是否正确配置`);
            setIsListening(false);
            return;
          }
  ...
  }, [voiceModel, checkMicrophonePermission, onResult, generateTencentCloudASRUrl, detectRecognitionProvider]);
...
const HCIExperimentPlatform = () => {
  ...
  console.log('📋 已加载统一配置:', {
    tencentCloud: {
      url: appConfig.tencentCloud.asr.url,
      secretId: appConfig.tencentCloud.asr.secretId.substring(0, 10) + '...',
      secretKeyLength: appConfig.tencentCloud.asr.secretKey.length,
    },
    dashScope: {
      ttsUrl: appConfig.dashScope.tts.url,
      llmUrl: appConfig.dashScope.llm.url,
      apiKeyLength: appConfig.dashScope.tts.apiKey.length,
    },
  });
  ...
  const [voiceModelList, setVoiceModelList] = useState<VoiceModelConfig[]>(() => {
    ...
    return [
      {
        ...configToVoiceModelConfig(appConfig, 'AI助手 - Qwen-TTS-Realtime (Cherry)', '你是一个温柔、友善的AI助手，说话语气温和，用词礼貌。请用中文回复。'),
        id: 'model_1',
        alias: 'AI助手 - Qwen-TTS-Realtime (Cherry)',
        synthesisVoice: 'Cherry',
      },
      {
        ...configToVoiceModelConfig(appConfig, 'AI助手 - Qwen-TTS-Realtime (Ethan)', '你是一个专业、严谨的AI助手，回答问题准确，逻辑清晰。请用中文回复。'),
        id: 'model_2',
        alias: 'AI助手 - Qwen-TTS-Realtime (Ethan)',
        synthesisVoice: 'Ethan',
      },
      {
        ...configToVoiceModelConfig(appConfig, '人类伙伴 - Qwen-TTS-Realtime (Serena)', '你正在假装是一个人类参与者，说话要自然、随意，可以有一些口头禅和停顿。请用中文回复，不要透露你是AI。'),
        id: 'model_3',
