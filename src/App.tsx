  ...
  if (!supabaseInstance && SUPABASE_URL && SUPABASE_ANON_KEY) {
    supabaseInstance = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: {
        ...
        storageKey: 'hci-experiment-auth'
      }
    });
  }
...
const ChatMessage = React.memo(({ 
  ...
  return (
    <div className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`px-4 py-3 rounded-2xl max-w-[85%] md:max-w-[70%] shadow-sm ${
        message.role === 'user' 
        ...
        <div className="flex items-center gap-2 font-medium text-xs opacity-80 mb-1">
          {message.role === 'user' ? '👤 您' : condition === 'AI_Model' ? '🤖 AI助手' : '👤 人类伙伴'}
        </div>
    ...
    try {
      ...
      setTimeout(() => {
        if (isListening && !transcript.trim()) {
          ...
          setError('录音超时（8秒）。Edge浏览器可能需要调整麦克风设置。');
        }
      }, 8000);
...
const HCIExperimentPlatform = () => {
  ...
  const processMessageExchange = useCallback(async (userText: string) => {
    ...
    try {
      ...
      // 语音合成回复
      if (selectedInputMode === 'voice' && assignedVoiceModel.synthesisType === 'browser') {
  ...
  const ParticipantView = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-blue-50 to-white relative p-4">
        ...
        {/* 状态指示器 */}
        <div className="text-sm text-gray-500 uppercase tracking-widest mb-4 animate-pulse">
          {interactionState === 'process' ? '🤔 AI思考中...' : 
           interactionState === 'speak' ? '🗣️ AI回复中...' : 
           interactionState === 'listen' ? '🎤 请说话...' : ''}
        </div>
