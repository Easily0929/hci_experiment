...
// --- 配置区域：请在这里填入你的 Supabase 信息 ---
// 为了安全，通常这些应该放在环境变量里。但对于实验原型，你可以直接填在这里。
// 这里的 key 是 "anon public" key，暴露在前端是相对安全的（配合RLS策略，或者仅做插入操作）
...
export default function HCIExperimentPlatform() {
  ...
  const uploadToCloud = async (msg: Message) => {
    ...
    // 这里的字段名必须和你 Supabase 里的 Table Column 完全一致
    const { error } = await supabase.from('experiment_logs').insert({
      ...
      timestamp: new Date(msg.timestamp).toISOString(), // 转换时间格式
    });
    ...
    recognition.onresult = async (event: any) => {
      ...
      try {
        ...
        const apiMessages = [
          systemMsg,
          ...newHistory.map((l) => ({
            role: l.role === 'partner' ? 'assistant' : 'user',
            content: l.content,
          })),
        ];
            ...
            >
              ...
              {!supabase && (
                <p className="text-xs mt-1 text-red-300">
                  Please set SUPABASE_URL and SUPABASE_ANON_KEY in the code to
                  enable cloud data collection.
                </p>
              )}
            </div>
